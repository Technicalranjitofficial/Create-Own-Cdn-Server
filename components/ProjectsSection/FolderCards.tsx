...
const FolderCards = ({data}:{data:IRequestFiles[]}) => {
  return (
    <div className='grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4  mb-20 gap-5 p-3'>
      {data.map((file,index)=>{
        return <Link href={`/projects/${file.name}`}> <div className='h-auto rounded-md text-slate-200 flex justify-between items-center w-full bg-transparent border border-gray-700 p-2 mt-5'>
